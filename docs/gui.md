...
By default the launcher opens the mean image, full movie, editable `rois` Labels layer, a top response-plot dock, and one tabbed right-side `twopy` dock. The first tab is Load, followed by Update, Plot, ROIs, Epochs, and Export. Use `--no-movie` to skip the movie preview, or `--movie-start` and `--movie-end` to choose a preview range.
