...
class NapariLoadedRecordingsTest(NapariAdapterTestCase):
    ...
    def test_loaded_recordings_panel_selects_and_unloads_layers(self) -> None:
        ...
        with temporary_directory() as temp_dir:
            ...
            self.assertIs(viewer.layers.selection.active, viewer.labels[0])
            self.assertEqual(viewer.labels[0].selected_label, 2)
            loaded_list.setCurrentRow(1)
            self.assertIs(viewer.layers.selection.active, viewer.labels[1])
            self.assertEqual(viewer.labels[1].selected_label, 1)
