...
- Napari response update controls now separate preview, reload, and persistence: "Recompute preview now" refreshes live plots in memory, "Reload saved analysis" reads existing analysis output from disk, and "Save ROIs + analysis" writes the current ROI file plus analysis HDF5, trial-level response CSV, and epoch-grouped response CSV under `exports/csvs/`.
